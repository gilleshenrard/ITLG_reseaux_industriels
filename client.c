...
/*
    ...
    freeDynList(&ds_list);
*/
    if((fd = open("data/makefile", O_WRONLY|O_CREAT)) == -1)
    {
    ...
    {
        //receive message from the server
        if (receiveData(sockfd, serialised, MAXDATASIZE, NULL, 1) == -1)
        {
            print_error("client: receiveData: %s", strerror(errno));
            return -1;
        }
        ...
        //unpack the data and store it
        if(write(fd, serialised, header.szelem) == -1)
        {
